...
var RoleWhere = struct {
	...
	ID:        whereHelperstring{field: "\"user_service\".\"role\".\"id\""},
	Slug:      whereHelperstring{field: "\"user_service\".\"role\".\"slug\""},
	CreatedAt: whereHelpernull_Time{field: "\"user_service\".\"role\".\"created_at\""},
}
...
var (
	roleAllColumns            = []string{"id", "slug", "created_at"}
	roleColumnsWithoutDefault = []string{"id", "slug"}
...
func (o *Role) RolePermissions(mods ...qm.QueryMod) rolePermissionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_service\".\"role_permission\".\"role_id\"=?", o.ID),
	)
...
func (o *Role) Users(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_service\".\"user\".\"role_id\"=?", o.ID),
	)
...
func (roleL) LoadRolePermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_service.role_permission.role_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_service.user`),
		qm.WhereIn(`user_service.user.role_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Role) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		if insert {
			...
			updateQuery := fmt.Sprintf(
				"UPDATE \"user_service\".\"user\" SET %s WHERE %s",
				strmangle.SetParamNames("\"", "\"", 1, []string{"role_id"}),
...
func Roles(mods ...qm.QueryMod) roleQuery {
	mods = append(mods, qm.From("\"user_service\".\"role\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"user_service\".\"role\".*"})
	}
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Role, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"user_service\".\"role\" where \"id\"=$1", sel,
	)
...
func (o *Role) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"user_service\".\"role\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"user_service\".\"role\" %sDEFAULT VALUES%s"
		}
...
func (o *Role) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"user_service\".\"role\" SET %s WHERE %s",
...
func (o RoleSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"user_service\".\"role\" SET %s WHERE %s",
...
func (o *Role) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"user_service\".\"role\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Role) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"user_service\".\"role\" WHERE \"id\"=$1"
...
func (o RoleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"user_service\".\"role\" WHERE " +
...
func (o *RoleSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"user_service\".\"role\".* FROM \"user_service\".\"role\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, rolePrimaryKeyColumns, len(*o))
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"user_service\".\"role\" where \"id\"=$1 limit 1)"
